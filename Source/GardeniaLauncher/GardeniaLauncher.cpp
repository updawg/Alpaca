...
void assertion(const char* pFormat, ...)
{
	...
	va_end(lArgs);
	MessageBox(0, msg, "Gardenia", MB_OK | MB_ICONASTERISK);
	exit(1);
}
...
bool installPlugY(HANDLE h, LPBYTE addr, char* libraryName, VersionUtility::Versions version)
{
	...
	if (!res || nb<6) assertion("Gardenia: Read process memory failed.");
	if (buf[0] != 0xFF || buf[1] != 0x15 || *(LPBYTE*)(buf + 2) != loadLibraryAddr)
		if (buf[0] != 0xE8 /*|| buf[1]!=0xD8 || buf[2]!=0x19*/ || buf[3] != 0x00 || buf[4] != 0x00 || buf[5] != 0x90)
			assertion("Gardenia: Read process memory failed.");
		else
	...
	if (!res || (nb != len)) assertion("Gardenia: Write custom data in memory failed");
	...
	*(DWORD*)(buf + 2) = (DWORD)loadDllAddr - (DWORD)loadCallerAddr - 6;
	...
	if (!res || (nb != len)) assertion("Gardenia: Write load library in memory failed");

	// Patch free library
	*(DWORD*)(buf + 2) = (DWORD)freeDllAddr - (DWORD)freeCallerAddr - 6;
	res = WriteProcessMemory(h, freeCallerAddr, buf, len, &nb);
	if (!res || (nb != len)) assertion("Gardenia: Write free library in memory failed");
...
bool launchGameXP(LPSTR commandLine, LPSTR currentDirectory, LPSTR libraryName, VersionUtility::Versions version)
{
	...
	while (WaitForDebugEvent(&DebugEvent, INFINITE))
	{
		...
		case EXCEPTION_DEBUG_EVENT:
			if (DebugEvent.u.Exception.ExceptionRecord.ExceptionCode == EXCEPTION_ACCESS_VIOLATION)
				MessageBox(0, "EXCEPTION_ACCESS_VIOLATION", "Gardenia", MB_OK | MB_ICONASTERISK);
			break;
	...
	MessageBox(0, "ERROR : Gardenia isn't installed", "Gardenia", MB_OK | MB_ICONASTERISK);
	CloseHandle(pi.hProcess);
	CloseHandle(pi.hThread);
	return false;
}
    ...
    )
{
	...
	// Check if Gardenia must be started.
	char libraryName[50];
	...
	// Check version
	if (version < VersionUtility::Versions::V107 || version > VersionUtility::Versions::V114d)
		assertion("Gardenia isn't compatible with this version : %s", VersionUtility::GetVersionAsString(version));
