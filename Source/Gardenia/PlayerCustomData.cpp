...
void Install_PlayerCustomData()
{
	...
	mem_seek(D2Common::GetOffsetByAddition(0x70462));
	MEMJ_REF4(Fog::D2AllocMem, init_PlayerCustomData);

	// update item
	mem_seek(D2Game::GetOffsetByAddition(0x10933));
	MEMC_REF4(D2GameGetObject, caller_updateItem_9);

	mem_seek(D2Game::GetOffsetByAddition(0x1097B));
	MEMC_REF4(D2GameGetObject, caller_updateItem_9);
	
	// Update client on loading
	mem_seek(D2Game::GetOffsetByAddition(0x23EB));
	memt_byte(0x5F, 0xE8);
	MEMT_REF4(0xC0335D5E, caller_updateClientPlayerOnLoading);

	// Free custom data.
	mem_seek(D2Common::GetOffsetByAddition(0x7055C));
	MEMJ_REF4(Fog::D2FreeMem, free_PlayerCustomData);

	// Free item in Stash (Server-side)
	mem_seek(D2Game::GetOffsetByAddition(0x7D12B));
	MEMJ_REF4(D2Common::D2UnitGetNextItem, callerServer_getNextItemToFree_9);

	// Free item in Stash (Client-side)
	mem_seek(D2Client::GetOffsetByAddition(0x8EF8F));
	MEMJ_REF4(D2Common::D2UnitGetNextItem, callerClient_getNextItemToFree_9);

	mem_seek(D2Game::GetOffsetByAddition(0x7D176));
	memt_byte(0x74, 0x90);
	memt_byte(0x35, 0x90);

	mem_seek(D2Client::GetOffsetByAddition(0x8F0CA));
	memt_byte(0x0F, 0x90);
	memt_byte(0x84, 0x90);
	memt_dword(0xBF, 0x90909090);

	mem_seek(D2Client::GetOffsetByAddition(0x8F13C));
	memt_byte(0x74, 0x90);
	memt_byte(0x6F, 0x90);
